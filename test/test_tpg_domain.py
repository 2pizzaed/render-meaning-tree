...
def test_parse_reasoning_jsonl_builds_structured_result() -> None:
    ...
    assert result.exceptions == [
        ReasoningException(id="node-1", result=False, exception_name="IllegalStateException")
    ]
...
def test_reasoning_result_str_is_human_readable() -> None:
    ...
    assert "Exceptions:\n  - id=node-1; result=False; exceptionName=IllegalStateException" in rendered
    assert 'Trace:\n  {\n    "branchResult": "Error",\n    "elements": []\n  }' in rendered
    assert "Variables:\n  a = first\n  z = last" in rendered
    assert 'Metrics:\n  solveTime: {\n      "seconds": 1.5,\n      "milliseconds": 1500.0\n    }' in rendered
...
def test_run_reasoner_cli_logs_jsonl_errors_as_readable_exception(monkeypatch, caplog) -> None:
    ...
    with caplog.at_level(logging.ERROR, logger="src.tpg_domain"):
        result = _run_reasoner_cli("reason", "domain", "generated.loqi", "--format", "jsonl")

    assert result is None
    assert "its_Reasoner CLI error java.lang.IllegalArgumentException: Failed requirement." in caplog.text
    assert "java.lang.IllegalArgumentException: Failed requirement.\n\tat demo.Main" in caplog.text
