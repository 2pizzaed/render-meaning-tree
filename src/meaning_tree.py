...
def _run_translator(input_file: Path, source_lang: str, target_lang: str = 'json') -> Optional[str]:
    ...
    try:
        result = subprocess.run(
            ['java', '-jar', str(jar_path), 'translate', 
             '--from', source_lang, '--to', target_lang, str(input_file)],
            capture_output=True,
            text=True,
            check=True
        )
